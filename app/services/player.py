...
def display_summary_stats(stats_df: pd.DataFrame, full_df: pd.DataFrame) -> None:
    ...
    if "shots_on" in frame.columns or "shots_faced" in frame.columns:
        ...
        goals_allowed = int(frame.get("goals_against", frame.get("goals_allowed", 0)).sum())
        ...
        shutouts = 0
        shutout_games = frame.loc[(result == "W") & (frame.get("goals_allowed", 0) == 0), "game_id"].unique()
...
def display_season_total_stats(stats: pd.DataFrame, games: pd.DataFrame, rosters: pd.DataFrame, team_id: int) -> None:
	...
	if "shots_on" in frame.columns or "shots_faced" in frame.columns:
		...
		df["shots_faced"] = df.get("shots_faced", df.get("shots_on", 0))
...
def _team_leaders(df: pd.DataFrame, pos: str):
	...
	st.session_state.leader["APP"] = _top3(df_active, lambda d: np.where(d["points"] > 0, d["assists"] / d["points"], 0))
